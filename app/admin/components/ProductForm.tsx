...
export default function ProductForm({ mode, initialData }: ProductFormProps) {
  ...
  const [soldCount, setSoldCount] = useState(initialData?.soldCount ?? 0);
    ...
    <div className="space-y-6">
      <section className="admin-panel rounded-[26px] px-6 py-7">
        <div className="flex items-center gap-4">
          <Link
            href="/admin/products"
            className="grid h-10 w-10 place-items-center rounded-xl border border-white/8"
          >
          ...
          <div>
            ...
            <p className="text-sm text-slate-400">
              {mode === 'create'
                ? 'Siapkan informasi produk baru'
                : 'Perbarui informasi produk yang tersimpan di database.'}
            </p>
          </div>
        </div>
      </section>
      ...
      <form onSubmit={handleSubmit} className="admin-panel space-y-6 rounded-3xl p-6">
        ...
        <div>
          <label className="text-sm text-slate-300">Jumlah Terjual</label>
          ...
          <p className="mt-2 text-xs text-slate-500">
            Isi angka penjualan secara manual. Contoh: 11150 akan ditampilkan sebagai 11,1RB+
            terjual.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 p-5">
          <label className="flex cursor-pointer items-center gap-3">
            ...
            <span className="text-sm font-medium text-slate-200">Aktifkan harga diskon</span>
          </label>

          {hasDiscount && (
            <div className="mt-5">
              <label className="text-sm text-slate-300">Harga Setelah Diskon</label>
        ...
        <button
          disabled={loading}
          className="inline-flex h-12 items-center gap-2 rounded-xl bg-sky-500 px-6 text-sm font-semibold text-white hover:bg-sky-400"
        >
